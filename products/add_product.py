...
def add_product_v3(data):
    # Проверка наличия цен и view_name
    if not any(sku["cnyPrice"] > 0 and "view_name" in sku for sku in data['skus']):
        return "Нет цен"
    ...
    # Очистка существующего продукта
    if not created:
        product.clear_all_fields()
        product.product_units.update(availability=False)
        product_slug = product.slug or f"{spu_id}_{property_id}_{manufacturer_sku}"
    else:
        product_slug = f"{spu_id}_{property_id}_{manufacturer_sku}"
    ...
    product.colorway = data['colorway']
    product.is_custom = data['custom']
    product.formatted_manufacturer_sku = manufacturer_sku
    product.manufacturer_sku = data.get("manufacturer_sku", "")
    product.actual_price = False
    ...
    # Обработка коллаборации
    if data["is_collab"] and data['collab_names']:
        collab, _ = Collab.objects.get_or_create(name=data['collab_names'][0])
        product.collab = collab
    ...
    # Обработка линеек
    product.lines.clear()
    for line_group in data['lines']:
        for line_name in line_group:
            line = Line.objects.get(name=line_name)
            product.lines.add(line)
            all_line_name = f"Все {line_name}"
            if Line.objects.filter(name=all_line_name).exists():
                product.lines.add(Line.objects.get(name=all_line_name))
    ...
    for brand_name in data['brands']:
        ...
        line, _ = Line.objects.get_or_create(name=brand_name)
        product.lines.add(line)
        all_brand_line = f"Все {brand_name}"
        if Line.objects.filter(name=all_brand_line).exists():
            product.lines.add(Line.objects.get(name=all_brand_line))

    # Обработка категорий
    product.categories.clear()
    for category_name in data['categories'][0]:
        category = Category.objects.get(name=category_name)
        product._add_category_with_parents(category)

    # Обработка изображений
    blacklisted_urls = product.black_bucket_link.values_list("url", flat=True)
    new_urls = [img for img in data["images"] if img not in blacklisted_urls]
    new_photos = [Photo(url=url) for url in new_urls]
    Photo.objects.bulk_create(new_photos)
    product.bucket_link.clear()
    product.bucket_link.add(*new_photos)
    ...
    for sku in data['skus']:
        ...
        self._process_sku(product, sku, data)
    ...
    product.available_sizes = self._get_available_sizes_info(product)
    product.one_update = True
    product.last_upd = timezone.now()
    product.available_flag = bool(product.bucket_link.exists())

    if created:
        self._update_product_score(product)
...
def _add_category_with_parents(self, product, category):
    """Рекурсивно добавляет категорию и всех ее родителей"""
    product.categories.add(category)

    # Добавление категории "Все/Вся ..."
    for prefix in ["Все", "Вся"]:
        all_category_name = f"{prefix} {category.name.lower()}"
        if Category.objects.filter(name=all_category_name).exists():
            product.categories.add(Category.objects.get(name=all_category_name))

    # Рекурсивное добавление родительских категорий
    if category.parent_category:
        self._add_category_with_parents(product, category.parent_category)


def _process_sku(self, product, sku, data):
    """Обрабатывает отдельный SKU продукта"""
    delivery_offers = get_ps_delivery_offers(sku, data)
    sizes, tables = self._get_size_info(sku)

    for delivery_offer in delivery_offers:
        delivery = self._create_delivery_type(sku, delivery_offer)
        product_unit = self._get_or_create_product_unit(product, sku, delivery_offer, delivery)
...
def _get_size_info(self, sku):
    """Получает информацию о размерах из SKU"""
    sizes = []
    tables = []
    ...
    if "table_name" in sku and 'filter_sizes' in sku:
        for size in sku["filter_sizes"]:
            if sku['table_name'] == "undefined":
                row = SizeTranslationRows.objects.filter(
                    is_one_size=True,
                    table__name="Один размер"
                ).first()
                sizes.append(row.id)
            else:
                table = SizeTable.objects.get(name=sku["table_name"])
                ...
                if size == 'undefined' or sku['row_name'] == "undefined":
                    ...
                    row = table.rows.filter(
                        row__contains={sku["row_name"]: size}
                    ).first()
                    if row:
                        sizes.append(row.id)
    else:
...
def _create_delivery_type(self, sku, delivery_offer):
    """Создает тип доставки"""
    days_min = delivery_offer['days_min'] + sku['delivery_info']['min_platform_delivery']
    days_max = delivery_offer['days_max'] + sku['delivery_info']['max_platform_delivery']
...
def _get_or_create_product_unit(self, product, sku, delivery_offer, delivery):
    """Получает или создает единицу продукта"""
    if product.product_units.filter(
            view_size_platform=sku['view_name'],
            delivery_type__delivery_type=delivery_offer['name']
    ).exists():
        product_unit = product.product_units.get(
            view_size_platform=sku['view_name'],
            delivery_type__delivery_type=delivery_offer['name']
        )
        product_unit.delivery_type.delete()
        ...
        product_unit = ProductUnit.objects.create(
            ...
            availability=True,
            currency=Currency.objects.get_or_create(name="CNY")[0],
            weight_kg=sku['weight']
        )
...
def _get_available_sizes_info(self, product):
    """Получает информацию о доступных размерах"""
    sizes_info = {"sizes": [], "filter_logo": ""}
    sizes_id = set()

    for unit in product.product_units.filter(availability=True):
        for size in unit.size.all():
            row = size.table.default_row
            if row.filter_name != "Один размер":
                if not sizes_info['filter_logo'] and row.filter_logo not in ['SIZE', "INT"]:
                    sizes_info['filter_logo'] = row.filter_logo
                if size.id not in sizes_id:
                    sizes_info['sizes'].append([size.id, f"{size.row[row.filter_name]}"])
                    sizes_id.add(size.id)

    sizes_info['sizes'] = [size[1] for size in sorted(sizes_info['sizes'])]
    return sizes_info


def _update_product_score(self, product):
    """Обновляет оценку продукта в зависимости от категории"""
    categories = product.categories.values_list("name", flat=True)
    if "Кеды" in categories or "Кроссовки" in categories:
        update_score_sneakers(product)
    else:
        update_score_clothes(product)